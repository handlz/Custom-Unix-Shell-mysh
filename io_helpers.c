 ...
 */
size_t tokenize_input(char *in_ptr, char **tokens) {//seperates the string by tabs and spaces and puts them into tokens[] 
    // TODO: Remove unused attribute DONE
    char *curr_ptr = strtok(in_ptr, DELIMITERS); //takes a string and spaces, returns
    size_t token_count = 0;

    while (curr_ptr != NULL) {  // TODO: Fix this DONE // strtok returns null when its reached the end
        // TODO: Fix this DONE
        tokens[token_count] = curr_ptr; //set index to word
        token_count += 1;
        curr_ptr = strtok(NULL, DELIMITERS); //curr_ptr is set to next node (just like a linked list lol)
    }
    tokens[token_count] = NULL; //sets last element as NULL
    return token_count;
}
//parameter: input string, array of char (string) pointers (this is where the tokens are stored)
